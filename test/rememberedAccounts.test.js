...
test('unmarkPasskey limpa a marca e remember cego mantem em false', () => {
  const storage = memoryStorage()
  const contas = createRememberedAccounts({ storage })

  contas.markPasskey('a@x.com')
  assert.equal(contas.list()[0].hasPasskey, true)

  assert.equal(contas.unmarkPasskey('A@X.com'), true)
  assert.equal(contas.list()[0].hasPasskey, false)

  // Sem marca a limpar, nao mexe no storage.
  assert.equal(contas.unmarkPasskey('a@x.com'), false)
