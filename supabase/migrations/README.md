...
| `20260919141317_v49_recurrence_end_check.sql` | Adiciona o CHECK `recurrence_end is null or recurrence_end >= date` em `public.transactions` como `NOT VALID` (AUDT-027), barrando intervalo invertido que hoje grava lançamento invisível; inventário, reparo e `VALIDATE CONSTRAINT` ficam em migration posterior. |
