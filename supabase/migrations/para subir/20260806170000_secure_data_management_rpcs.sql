-- MIGRATION: Exige token valido e AAL/MFA nas RPCs SECURITY DEFINER de dados.
-- OBJETIVO: Impedir que uma sessao AAL1 ou revogada importe/apague dados.
-- PRE-CONDICOES: As funcoes public.is_token_valid() e public.has_required_aal()
-- ja existem, com EXECUTE concedido a authenticated.
-- COMPATIBILIDADE: Mantem assinaturas, grants e o uso de auth.uid(); nenhum dado
-- e migrado. Sessoes validas continuam usando as RPCs sem alteracao no frontend.
-- IMPACTO RLS/GRANTS/INDICES: SECURITY DEFINER permanece necessario para a
-- operacao atomica, mas passa a aplicar explicitamente as mesmas garantias das RLS.
-- VERIFICACAO: Em homologacao, testar AAL2 e usuario sem MFA (sucesso), AAL1 de
-- usuario com MFA e token revogado (erro, sem alterar dados).
-- RECUPERACAO: Se houver regressao confirmada, crie nova migracao corretiva;
-- nao remova esta migracao nem relaxe RLS.
