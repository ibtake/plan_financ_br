  ...
  return <form className="card" onSubmit={submit}>
    ...
    <div className="form-grid" style={{ marginTop: 16 }}>
      ...
      {reverse ? <>
        ...
        <div className="field"><label className="label">Já guardado</label><input className="input mono" value={form.current} onChange={(event) => set({ current: event.target.value })} inputMode="decimal" readOnly={Boolean(initial)} /><small className="text-xs text-muted">{initial ? 'Edite os aportes em Mais detalhes.' : ''}</small></div>
