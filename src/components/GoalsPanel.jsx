...
import { amountToInput, formatAmountInput, formatCurrency, formatDate, formatPercent, parseAmount } from '../utils/format.js'
...
function GoalForm({ initial, onSave, onCancel }) {
  ...
  return <form className="card" onSubmit={submit}>
    ...
    <div className="form-grid" style={{ marginTop: 16 }}>
      ...
      {reverse ? <>
        <div className="field"><label className="label">Valor inicial *</label><input className="input mono" value={form.originalAmount} onChange={(event) => set({ originalAmount: formatAmountInput(event.target.value) })} inputMode="numeric" /></div>
        <div className="field"><label className="label">Já recomposto</label><input className="input mono" value={form.initialContribution} onChange={(event) => set({ initialContribution: formatAmountInput(event.target.value) })} inputMode="numeric" /></div>
        ...
        <div className="field"><label className="label">Valor alvo *</label><input className="input mono" value={form.target} onChange={(event) => set({ target: formatAmountInput(event.target.value) })} inputMode="numeric" /></div>
        <div className="field"><label className="label">Já guardado</label><input className="input mono" value={form.current} onChange={(event) => set({ current: formatAmountInput(event.target.value) })} inputMode="numeric" readOnly={Boolean(initial)} /><small className="text-xs text-muted">{initial ? 'Edite os aportes em Mais detalhes.' : ''}</small></div>
