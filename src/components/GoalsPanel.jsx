...
function GoalForm({ initial, onSave, onCancel }) {
  ...
  return <form className="card" onSubmit={submit}>
    ...
    <div className="form-grid" style={{ marginTop: 16 }}>
      <div className="field span-2"><label className="label" htmlFor="goal-name">Nome *</label><input id="goal-name" name="goal-name" className="input" value={form.name} onChange={(event) => set({ name: event.target.value })} required /></div>
      {reverse ? <>
        ...
        <div className="field"><label className="label" htmlFor="goal-current">Já guardado</label><input id="goal-current" name="goal-current" className="input mono" value={form.current} onChange={(event) => set({ current: formatAmountInput(event.target.value) })} inputMode="numeric" readOnly={Boolean(initial)} /><small className="text-xs text-muted">{initial ? 'Edite os aportes em Mais detalhes.' : ''}</small></div>
        <div className="field"><label className="label" htmlFor="goal-deadline">Prazo</label><input id="goal-deadline" name="goal-deadline" className="input goal-date-input" type="date" value={form.deadline} onChange={(event) => set({ deadline: event.target.value })} /></div>
      </>}
      <div className="field span-2"><label className="label">Ícone e cor</label><div className="swatch-grid">{ICONS.map((icon) => <button type="button" className={`swatch${form.icon === icon ? ' selected' : ''}`} onClick={() => set({ icon })} aria-label={`Ícone ${icon}`} aria-pressed={form.icon === icon} key={icon}><AppIcon emoji={icon} /></button>)}</div><div className="swatch-grid">{COLORS.map((color) => <button type="button" className={`color-dot${form.color === color ? ' selected' : ''}`} onClick={() => set({ color })} style={{ background: color }} aria-label={`Cor ${color}`} aria-pressed={form.color === color} key={color} />)}</div></div>
    </div>
...
function ReverseMetadataForm({ initial, onSave, onCancel }) {
  ...
  return <form className="card" onSubmit={submit}>
    ...
    <div className="form-grid"><div className="field span-2"><label className="label" htmlFor="reverse-goal-name">Nome *</label><input id="reverse-goal-name" name="reverse-goal-name" className="input" value={form.name} onChange={(event) => set({ name: event.target.value })} required autoFocus /></div><div className="field span-2"><label className="label">Ícone e cor</label><div className="swatch-grid">{ICONS.map((icon) => <button type="button" className={`swatch${form.icon === icon ? ' selected' : ''}`} onClick={() => set({ icon })} aria-label={`Ícone ${icon}`} aria-pressed={form.icon === icon} key={icon}><AppIcon emoji={icon} /></button>)}</div><div className="swatch-grid">{COLORS.map((color) => <button type="button" className={`color-dot${form.color === color ? ' selected' : ''}`} onClick={() => set({ color })} style={{ background: color }} aria-label={`Cor ${color}`} aria-pressed={form.color === color} key={color} />)}</div></div></div>
...
function EditContributionModal({ item, minDate, onSave, onClose }) {
  ...
  return <dialog open className={`reverse-modal-backdrop${closing ? ' is-closing' : ''}`} onClick={(event) => { if (event.target === event.currentTarget) close() }} aria-labelledby="edit-contribution-title">
    <form ref={surfaceRef} className={`reverse-details aporte-modal${closing ? ' is-closing' : ''}`} onSubmit={submit} aria-labelledby="edit-contribution-title">
...
function StandardCard({ goal, contributions = [], onAddContribution, onUpdateContribution, onEdit, onDelete, isDeleting }) {
  ...
  return (
    <div className={`goal-card goal-card-expandable${completed ? ' goal-completed' : ''}`} onClick={toggleFromCard}>
      <div className="row-between">
        <button className="standard-card-main" onClick={() => setOpen(!open)} disabled={isDeleting}>
          <div className="goal-icon" style={{ background: `${goal.color}22`, color: goal.color }}><AppIcon emoji={goal.icon} /></div>
          <span><strong>{goal.name}</strong><small className="goal-badge standard-badge">Meta</small></span>
        </button>
        <div className="row">
      ...
      <div className="progress">
        <div
          ...
          aria-valuemin="0"
          aria-valuemax="100"
          aria-valuenow={Math.min(100, Math.max(0, Number(progress) || 0))}
          aria-valuetext={formatPercent(progress, 1)}
        />
      </div>
      <div className="completion-status text-xs"><span>{formatPercent(progress, 1)} concluído</span>{completed && <span className="completion-trophy" role="img" aria-label="Meta concluída">🏆</span>}</div>
      {open && <>
        <div className="standard-summary"><span>Valor da meta<strong>{formatCurrency(goal.target)}</strong></span><span>Já aportado<strong>{formatCurrency(goal.current)}</strong></span></div>
        {!completed && <button className="btn btn-primary" onClick={() => setModal(true)} disabled={isDeleting}><Plus size={15} />Aportar</button>}
        <button className="btn btn-sm" type="button" onClick={() => setDetails(true)} disabled={isDeleting}>Mais detalhes</button>
      </>}
      {modal && <StandardAporteModal goal={goal} onSave={onAddContribution} onClose={() => setModal(false)} />}
      {editingItem && <EditContributionModal item={editingItem} onSave={onUpdateContribution} onClose={() => setEditingItem(null)} />}
      {details && <div className={`reverse-modal-backdrop${detailsDialog.closing ? ' is-closing' : ''}`} onMouseDown={detailsDialog.close} role="presentation">
        <section ref={detailsDialog.surfaceRef} className={`reverse-details${detailsDialog.closing ? ' is-closing' : ''}`} onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="reverse-details-title">
          <div className="row-between"><strong id="reverse-details-title">Mais detalhes</strong><button className="icon-btn" aria-label="Fechar" onClick={detailsDialog.close}><X size={18} /></button></div>
