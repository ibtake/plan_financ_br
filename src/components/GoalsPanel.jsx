...
function ReverseCard({ goal, onAporte, onEdit, onDelete, history = [], contributions = [], events = [], onUpdateContribution, isDeleting }) {
  ...
  return <div className={`goal-card reverse-goal-card${goal.reverseCompletedAt || goal.reverseRemainingAmount <= 0 ? ' goal-completed' : ''}`}>
    ...
    <div className="text-xs">{formatPercent(progress, 1)} concluído</div>
...
function StandardCard({ goal, onUpdate, onEdit, onDelete, isDeleting }) {
  ...
  return <div className={`goal-card${goal.current >= goal.target ? ' goal-completed' : ''}`}><div className="row-between"><button className="standard-card-main" onClick={() => setOpen(!open)} disabled={isDeleting}><div className="goal-icon" style={{ background: `${goal.color}22`, color: goal.color }}><AppIcon emoji={goal.icon} /></div><span><strong>{goal.name}</strong><small className="goal-badge standard-badge">Meta</small></span></button><div className="row"><button className="icon-btn" onClick={() => onEdit(goal)} disabled={isDeleting}><Pencil size={15} /></button><button className="icon-btn danger" disabled={isDeleting} onClick={async () => { if (window.confirm('Excluir esta meta?')) await onDelete(goal.id) }}><Trash2 size={15} /></button></div></div><div className="progress"><div className="progress-bar" style={{ width: `${Math.min(100, progress)}%`, background: goal.color }} /></div><div className="text-xs">{formatPercent(progress, 1)} concluído</div>{open && <form className="row" onSubmit={(event) => { event.preventDefault(); const value = parseAmount(amount); if (value) { onUpdate(goal.id, { current: goal.current + value }); setAmount('') } }}><input className="input mono grow" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Aportar R$..." disabled={isDeleting} /><button className="btn btn-primary btn-sm" disabled={isDeleting}><Plus size={15} />Aportar</button></form>}</div>
}
