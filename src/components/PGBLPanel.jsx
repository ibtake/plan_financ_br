...
export function calculatePGBL(months, premise, params) {
  ...
  const simplificado = Math.min(
    ...
    params.tetoDescontoSimplificado,
  )
...
function MoneyField({ label, value, onChange }) {
  ...
  return <label className="field"><span className="label">{label}</span><input className="input" type="text" inputMode="numeric" value={draft} onFocus={() => setEditing(true)} onChange={(event) => { const masked = formatAmountInput(event.target.value); setDraft(masked); onChange(parseAmount(masked)) }} onBlur={() => { setEditing(false); setDraft(value === '' || value == null ? '' : amountToInput(value)) }} /></label>
}
